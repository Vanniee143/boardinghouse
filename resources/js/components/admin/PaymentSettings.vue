  ...
  methods: {
    ...
    async saveQRCode() {
      try {
        ...
        formData.append('created_by', this.qrData.created_by);

        const response = await axios.post('/admin/payment-qr-codes', formData, {
          headers: {
    ...
    async deleteQRCode(id) {
      ...
      try {
        const response = await axios.delete(`/admin/payment-qr-codes/${id}`);
        if (response.data.status === 'success') {
...
/* Add these styles to improve the select appearance */
.form-group select {
  width: 100%;
  padding: 0.75rem;
  border: 1px solid #ddd;
...
.form-group select option[value=""] {
  color: #666; /* Color for the placeholder option */
}
